...
public class FarmStorage extends Storage{
	
	@Override
	public void load(){
		this.config = new Config(new File(Main.getInstance().getDataFolder(), "crops.yml"), Config.YAML);
		...
		for(Map.Entry<String, Long> entry : this.entrySet()){
			if(entry.getValue() < System.currentTimeMillis()){
				...
				if(CropControl.isCrop(pos)){
					if(CropControl.updateCrop(pos)){
						toAdd.add(entry.getKey()); //grow continously
